...
import { faBuilding, faStar, faStarHalf, faChevronLeft, faChevronRight, faPaperPlane, faWifi, faDesktop, faPen, faWheelchair, faPrint, faVideo, faUtensils, faChild, faDog, faChalkboard, faVideoCamera } from '@fortawesome/free-solid-svg-icons';
...
const SpazioNelDettaglio = () => {
    ...
    const [dailyPrice, setDailyPrice] = useState(50); // Prezzo giornaliero
                    ...
                    <div className={`flex flex-col gap-5 h-full w-2/5`}>
                        ...
                        <div id="commentsSection" className={`w-full flex flex-col overflow-hidden rounded-2xl bg-stone-100 border-1 border-stone-900/10 shadow-sm relative h-2/5`}>
                            ...
                            <div className='flex justify-between gap-2 p-2'>
                                ...
                                <div className='flex items-center h-10 text-xl rounded-xl bg-stone-100 border-1 border-stone-900/10 shadow-sm px-2'>
                                    {[1, 2, 3, 4, 5].map((star) => (
                                        <span
                                            key={star}
                                            ...
                                            className='cursor-pointer transition-transform duration-150 ease-out active:scale-125'>
                                            <FontAwesomeIcon
                                ...
                                <button className='aspect-square size-10 rounded-xl bg-stone-900 text-stone-100 border-1 border-stone-900/10 hover:bg-west-side-500 hover:text-stone-100'>
                            ...
                            <div className="relative h-full flex flex-col p-4 gap-5 overflow-y-auto rounded-2xl">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    {features.map((feature, index) => (
                                        <button
                                            key={index}
                                            className='px-3 py-1 text-sm rounded-full hover:bg-west-side-100 border-1 hover:border-west-side-300 hover:text-west-side-900 transition duration-500 hover:duration-150 delay-250 hover:delay-0'>
                                ...
                                <div className='w-full mt-auto p-4 rounded-lg flex gap-5 border-1 border-stone-900/10 bg-stone-100 shadow-sm transition duration-500'>
                                    ...
                                    <div className='w-full flex flex-col gap-2 relative'>
                                        <button
                                            className="absolute top-2 right-2 py-2 px-4 bg-west-side-500 text-white font-bold rounded-lg hover:bg-west-side-600 transition"
                                            onClick={() => alert('Prenotazione effettuata!')}>
                                            Prenota Ora
                                        </button>
                                        <p className="text-lg">
                                            <strong>Prezzo totale:</strong> {dailyPrice}€ * {selectedDates.size} = {calculateTotalPrice()}€
                                        </p>
                                        <div>
                                            <p className="text-lg">
                                                <strong>Giornate e posti disponibili:</strong>
                                            </p>
