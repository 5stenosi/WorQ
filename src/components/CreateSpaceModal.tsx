...
import { faPlus, faTrashCan, faImages, faXmark, faWifi, faDesktop, faPen, faWheelchair, faPrint, faVideo, faUtensils, faChild, faDog, faChalkboard, faVideoCamera } from '@fortawesome/free-solid-svg-icons';
...
const CreateSpaceModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
    ...
    return (
        <div
            className={`fixed inset-0 flex items-center justify-center z-9999 transition-opacity duration-300 ${isOpen && !isClosing ? 'opacity-100' : 'opacity-0'
                }`}
        >
            <div
            ...
            ></div>
            <div
                className={`bg-stone-100 rounded-lg shadow-lg max-w-5xl w-full p-5 relative transition-transform duration-300 ${isOpen && !isClosing ? 'scale-100' : 'scale-90'}`}>
                ...
                <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
                    <div className="flex gap-5">
                        ...
                        <div className="flex flex-col w-2/5 gap-5">
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="relative h-52 border rounded-lg border-stone-300 flex items-center justify-center cursor-pointer overflow-hidden group">
                                    ...
                                    <label
                                        ...
                                        className={`flex items-center gap-2 px-3 py-1 text-sm rounded-full border-1 cursor-pointer transition duration-500 hover:duration-150 delay-250 hover:delay-0
                                            ${selectedServices.includes(service.id)
                                                ...
                                                : 'hover:bg-west-side-100 hover:border-west-side-300 hover:text-west-side-900 border-stone-300 text-stone-500'
                                            }`}>
                                        <input
                                            type="checkbox"
                                            id={service.id}
                        ...
                        <div className="flex flex-col w-3/5 gap-5">
                                ...
                                <div className="w-full flex flex-col">
                                    ...
                                    <div className="flex gap-3">
                                        <input
                                            ...
                                            onChange={handleInputChange}
                                            className="w-full p-2 border rounded-lg border-stone-300 focus:outline-none focus:ring-2 focus:ring-west-side-500"
                                            placeholder="Enter price per day"
                                            min="1" />
                                        <span className="flex justify-center items-center aspect-square rounded-lg h-full border-1 text-xl">€</span>
                                    </div>
                                </div>
                            </div>
                            {/* Space Type */}
                            ...
                            <div className="flex flex-col h-full">
                                ...
                                <textarea
                                    ...
                                    onChange={handleInputChange}
                                    className="h-full p-2 border rounded-lg border-stone-300 focus:outline-none focus:ring-2 focus:ring-west-side-500"
                    ...
                    <div className="flex justify-between mt-3">
                        <button
                            ...
                            className="overflow-hidden flex justify-start items-center rounded-xl bg-red-500 text-stone-100 transition-all duration-150 w-12 hover:w-44 ease-out active:scale-90 hover:scale-110 origin-left">
                        ...
                        <button
                            type="submit"
                            className="overflow-hidden flex justify-end items-center rounded-xl bg-stone-900 hover:w-44 hover:scale-110 text-stone-100 transition-all duration-150 w-12 ease-out active:scale-90 origin-right">
                ...
                <button
                    className="absolute size-10 top-4 right-4 bg-stone-100 hover:bg-red-500 border-1 border-stone-900/10 rounded-md shadow-sm text-stone-900 hover:text-stone-100 text-2xl transition"
                    onClick={() => {
                        setIsClosing(true);
                        setTimeout(onClose, 300); // Synchronize with the transition duration
                    }}
